...
const QuizInstruction = () => {
  ...
  const quizData = {
    heading: 'Quiz:',
    topic: lessons && lessons[0]?.title,
    name: user?.user?.user?.fullName,
    subject: subject?.subjectDetails[0]?.subject?.mainSubjectId?.name,
    instruction:
      lessons &&
      `You are about to that a ${lessons?.questions?.length} question(s) quiz to test how much you understand ${lessons[0]?.title}.`,
  }
      ...
      <div className="quizContainer">
        ...
        <div id="quizSection">
          ...
          <div className="quizSubject">
            Subject :{' '}
            <span className="instructionSubject p-2"> {quizData.subject}</span>
          </div>
