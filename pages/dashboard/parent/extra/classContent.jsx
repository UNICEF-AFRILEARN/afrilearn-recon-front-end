...
const ClassContent = ({ courseContext, children }) => {
  ...
  const mappedCoursesCount = myChildren?.map(
    (children) => children.enrolledCourses,
  );

  const classList = mappedCoursesCount
    ?.filter((filterClass) => filterClass.length > 0)
    .map((mappinClass) => mappinClass[0].courseId.name);

  let sortclassId = mappedCoursesCount
    ?.filter((filterClass) => filterClass.length > 0)
    .map((mappinClass) => mappinClass);
  ...
  let sortedClass_ids = [];
  const sortClassIds = () => {
    sortedClassIds = sortclassId
      .filter((sortIds) => sortIds)
      .map((sort_ids) => sort_ids);
        ...
        <div className={styles.labelwrapper}>
          <ul>
            ...
            >
              ...
              {classList &&
                classList.map((childClass) => (
                  <option placeholder="Select a Role">{childClass}</option>
                ))}
            </select>
            <li>
              Class note <BiChevronDown />{" "}
            </li>
          </ul>
        </div>
      </div>
